    ...
    @if ($selectedConversation)
        <div class="chatbox_header">
            ...
            <div class="img_container">
                <img src="https://ui-avatars.com/api/?name={{ $receiver_instance->name }}" alt="chat photo">
            </div>
                            ...
                            @php
                                if($message->user->id === auth()->id()) {
                                    if($message->read == 0) {
                                        ...
                                        echo '<i class="bi bi-check2-all text-white "></i>';
                                    }
                                }
                            @endphp
    ...
    <script>
        window.addEventListener('markMessageAsRead', event=>{
            ...
            value.array.forEach(element, index => {
                element.classList.remove('bi bi-ckeck2');
                element.classList.add('bi bi-check2-all', 'text-white');
            });
        });
    </script>
</div>
